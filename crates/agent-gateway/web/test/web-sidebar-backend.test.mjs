...
test("subscribeConnection follows the authenticated gateway socket", () => {
  const { api, state, emitConnection } = createFakeApi();
  const activityStore = createActivityStore();
  const backend = createWebSidebarBackend({
    api,
    activityStore,
  ...
  emitConnection(true);
  assert.deepEqual(events, [true], "duplicate socket states are deduped");
